...
export const run = (api_surface: Promise<Response>) => {
  api_surface
    ...
    .then((json) => {
      ...
      fetch(
        "http://localhost:8000/features/cross_species?seed_species=human&seed_side=left&seed_vertex=1",
      )
        .then((response) => {
          return response.json();
        })
